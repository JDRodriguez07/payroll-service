...
public class PayrollServiceTest {
    ...
    void generateMonthlyPayrolls_ThrowsException_IfNotLastBusinessDay() {
        ...
        try (MockedStatic<LocalDate> mockDate = mockStatic(LocalDate.class)) {
            ...
            assertThrows(Exception.class, () -> payrollService.generateMonthlyPayrolls());
        }
    }
    ...
    void generateMonthlyPayrolls_CreatesPayrollsAndDeductions() {
        ...
        try (MockedStatic<LocalDate> mockDate = mockStatic(LocalDate.class)) {
            ...
            healthType.setPercentage(new BigDecimal("0.05"));
            healthType.setName("Salud");
    
            when(deductionTypeRepository.findByNameIgnoreCase("Pension"))
               .thenReturn(Optional.of(pensionType));
    
            when(deductionTypeRepository.findByNameIgnoreCase("Salud"))
               .thenReturn(Optional.of(healthType));
    
            assertDoesNotThrow(() -> payrollService.generateMonthlyPayrolls());
        }
    }
